...
void galapagos::net::accept_handler(const boost::system::error_code &e){
        ...
        while(1){
            ...
            {   
                boost::asio::ip::tcp::acceptor a(io_service, boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), GALAPAGOS_PORT));
                {
                    ...
                    std::lock_guard <std::mutex> guard(mutex);
                    a.async_accept(*sock_vect[sock_vect.size() -1], boost::bind(&galapagos::net::accept_handler, this, boost::asio::placeholders::error));
                }
