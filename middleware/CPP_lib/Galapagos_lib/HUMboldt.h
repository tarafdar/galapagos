#ifndef __HUMBOLDT_H__   // if x.h hasn't been included yet...
#define __HUMBOLDT_H__
...
union TCP_envelope{
	...
	unsigned char buffer[TCP_ENVELOPE_SIZE];
};
...
{
	TCP_envelope TCP_envlp;
	char buffer[TCP_DATA_BUFFER_SIZE];
};

struct ETH_data_packet
{
	ETH_envelope ETH_envlp;
