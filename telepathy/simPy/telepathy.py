...
#            self.configCmdFPGA(rank_id)
    ...
    def configParametersFPGA(self, rank_id):
        ...
        binToSend = bytearray(long(len(self.parameters['neural_net'][rank_id]['configParameters'])))
        binToSend.extend(bytearray(long(self.parameters['neural_net'][rank_id]['biases_weights_baseaddr'])))

        #send configuration parameters to all fpga_ranks
        ##TODO ONLY ONE RANK RIGHT NOW 
        GalapagosNet.binToStream(self, binToSend, self.parameters['neural_net'][rank_id]['dst_rank'])
        GalapagosNet.binToStream(self, self.parameters['neural_net'][rank_id]['configParameters'], 'DMAParameters', self.parameters['neural_net'][rank_id]['dst_rank']) 
    
    def configDMAFPGA(self, rank_id):
       
        binToSend = bytearray(long(self.parameters['neural_net'][rank_id]['ifm_baseaddr']))
        binToSend.extend(bytearray(long(self.parameters['neural_net'][rank_id]['ifm_packet_length'] * 2)))
        binToSend.extend(bytearray(long(self.parameters['neural_net'][rank_id]['ofm_baseaddr'])))
        binToSend.extend(bytearray(long(self.parameters['neural_net'][rank_id]['ofm_packet_length'] * 2)))
